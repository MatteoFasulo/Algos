...
GOOGLE_API = "AIzaSyCGrc8w7t200FtQu8lIZsSRZVjnAFK2cPo"
...
def make_capoluoghi_regionali(db_input: str = f"data{os.sep}comuni_geo final.json",
                              filename: str = f"data{os.sep}capoluoghi_regione.json"):
    """
    ...
    for regione in regioni:
        ...
        capoluoghi_regionali[regione]["sigla_provincia"] = \
        my_df[my_df["comune"] == capoluogo_regionale]["sigla_provincia"].values[0]
        capoluoghi_regionali[regione]["lng"] = np.float64(
            my_df[my_df["comune"] == capoluogo_regionale]["lng"].values[0]).item()
        capoluoghi_regionali[regione]["lat"] = np.float64(
            my_df[my_df["comune"] == capoluogo_regionale]["lat"].values[0]).item()
...
def make_capoluoghi_provincia(db_input: str = f"data{os.sep}comuni_geo final.json",
                              db_capoluoghi: str = f"data{os.sep}capoluoghi_regione.json",
                              filename: str = f"data{os.sep}capoluoghi_provincia-t.json"):
    """
...
def compute_weights(cities: list, filename: str, start: int = 0, stop: int = None, traffic_model="best_guess",
    ...
    try:
        ...
        for i in range(start, stop):
            ...
            for slot in chunks:
                ...
                # print(slot,"\n")
                result = gmaps.distance_matrix(origins[i], slot, mode='driving', departure_time=night_time,
...
def parallel_scraping(comuni, start=0, stop=None):
    if stop == None:
        stop = len(comuni)
    group = chunk([x for x in range(start, stop)], 10)  # Numero città
    group = chunk(group, 12)  # Numero thread
    print(group)
    filename = ''
    for slot in group:
        for i in slot:
