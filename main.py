    ...
    for i in range(len(cities)):
        ...
        for j in range(len(cities)):
            ...
            result = gmaps.distance_matrix(origins, destination, mode='driving', departure_time=night_time,
                                           traffic_model=traffic_model)["rows"][0]["elements"][0]["distance"]["value"]
            result = round(result / 1000)
            distances.append(result)
            time = gmaps.distance_matrix(origins, destination, mode='driving', departure_time=night_time,
                                         traffic_model=traffic_model)["rows"][0]["elements"][0]["duration"]["value"]
            time = round(time / 60)
...
if __name__ == "__main__":
    ...
    # fill_coords(unique_cities(), "pesi")
